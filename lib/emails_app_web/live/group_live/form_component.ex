defmodule EmailsAppWeb.GroupLive.FormComponent do
  ...
  def update(%{group: group} = assigns, socket) do
    changeset = UserAction.change_group(group)
  ...
  def handle_event("validate", %{"group" => group_params}, socket) do
    changeset =
      ...
      |> UserAction.change_group(group_params)
  ...
  defp save_group(socket, :edit, group_params) do
    case UserAction.update_group(socket.assigns.group, group_params) do
      {:ok, group} ->
  ...
  defp save_group(socket, :new, group_params) do
    case UserAction.create_group(group_params) do
      {:ok, group} ->
        notify_parent({:saved, group})
